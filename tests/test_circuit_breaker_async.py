...
@pytest.mark.asyncio
async def test_call_async_basic_functionality():
...
@pytest.mark.asyncio
async def test_call_async_failure_handling():
...
@pytest.mark.asyncio
async def test_call_async_circuit_breaker_error():
...
@pytest.mark.asyncio
async def test_call_async_recovery():
    ...
    # Cause circuit to open
        ...
        try:
            result = await cb.call_async(successful_func)
            assert result == "success"
            assert cb.state == cb.State.CLOSED  # Should transition back to closed after successful half-open call
        except Exception as e:
            assert False, f"Should have succeeded: {e}"
            
            # Need to import asyncio for sleep in tests
            import asyncio